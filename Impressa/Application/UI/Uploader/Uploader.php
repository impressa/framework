...
class Uploader extends Control {
	...
	public function handleUpload($fileOwnerId = NULL) {
		$httpResponse = $this->presenter->context->httpResponse;

		$httpResponse->addHeader('Expires', 'Mon, 26 Jul 1997 05:00:00 GMT');
		...
		// Settings
		$targetDir = $this->presenter->context->params['tempDir'] . DIRECTORY_SEPARATOR . "plupload";
		//$targetDir = 'uploads';
		...
		if (!$chunks || $chunk == $chunks - 1) {
			...
			$fileOwnerId = isset($_REQUEST["fileOwnerId"]) ? $_REQUEST["fileOwnerId"] : NULL;
			$this->onUpload($file, $fileOwnerId);
		...
		$this->presenter->payload->result = null;
		$this->presenter->payload->id = 'id';
		$this->presenter->payload->jsonrpc = '2.0';
